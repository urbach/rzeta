...
QZeta <- function(qsq, l=0, m=0, dvec=c(0,0,0), gamma=1, A=1, tol=0.000001, Lmax=6) {
  ...
  return(.Call( "QZetaFunction",    as.double(qsq), as.integer(n), as.integer(l[1]), as.integer(m[1]), as.integer(dvec[1:3]),  as.double(gamma), as.double(A), as.double(tol), as.integer(Lmax) ) )
}
