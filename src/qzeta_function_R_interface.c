...
SEXP QZetaFunction(SEXP qsq_, SEXP n_, SEXP l_, SEXP m_, SEXP dvec_, SEXP gamma_, SEXP A_, SEXP tol_, SEXP Lmax_ ) {
/*  int qzeta_function (double z[2], double q2, int l, int m, int*dvec, double gamma, double A, double epsAbs, double epsRel, int Lmax); */
  ...
  for(int i = 0; i < n; i ++) {
    ...
    if(ISNAN(qsq[i]) || ISNA(qsq[i])) {
      ...
      exitstatus = qzeta_function (ires, qsq[i], l, m, dvec, gamma[i], A[i], tol, 1.e12, Lmax);
      resp[i].r = ires[0];
      resp[i].i = ires[1];
    }

    /* check for anything went wrong in qzeta_function evaluation */
    if( exitstatus != 0 ) {
      warning("[QZetaFunction] Error from qzeta_function for momentum number %d, status was %d\n", i, exitstatus);
      resp[i].r = NA_REAL;
      resp[i].i = NA_REAL;
    }
  }
